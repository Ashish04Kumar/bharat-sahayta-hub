...
const RegisterHelperForm: React.FC<RegisterHelperFormProps> = ({
    ...
    <div className="w-full bg-white border border-input shadow-md rounded-xl overflow-hidden mt-4 p-6 pt-8">
      <form className="register-helper-form">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {registerHelperTranslationData &&
            Object.entries(registerHelperTranslationData?.formFields)
              .sort(([, a], [, b]) => a.sequence - b.sequence)
              .map(([, field], idx) => {
                const IconComponent = getFieldIcon(field.name);
                return (
                  <div className="form-group space-y-2" key={idx}>
...
RegisterHelperForm.displayName = "RegisterHelperForm";
